...
char *get_env(char *str, env_list *env)
{
	...
	{
		j = 0;
		while ((env->env_variable)[j] == str[j]) /* find desired env variable */
			j++;
		if (str[j] == '\0' && (env->env_variable)[j] == '=')
			break;
		env = env->next;
	}
	...
	return (c_strdup(env->env_variable, cs)); /* make a copy of variable w/o title */
}
